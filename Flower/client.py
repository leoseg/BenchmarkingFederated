...
parser.add_argument(
    "--random_state",
    type=int,
    help="flag for setting the random state for train test",
    default=1,
)
parser.add_argument(
    "--unweighted",
    type=bool,
    help="flag for setting if data is unweighted",
    default=False,
)
parser.add_argument(
...
train_ds, test_ds = df_train_test_dataset(
    df,
    kfold_num=args.random_state,
    random_state=args.run_repeat,
    label=configs.get("label"),
    scale=configs.get("scale"),
    unweighted=args.unweighted,
)
print(
    "Loading data backend dataset of client has num of examples", train_ds.cardinality()
)
...
class Client(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        preprocessed_ds = preprocess(
            train_ds, epochs=config["local_epochs"], seed=config["server_round"]
        )
        ...
        if args.system_metrics and args.client_index == 0:
            tf.print(
                ...
                output_stream=f"file://{flw_time_logging_directory}",
            )
