...
echo "Starting flwr experiment with num clients ${NUM_CLIENTS} and noise ${NOISE} num rounds ${NUM_ROUNDS} and data ${DATA_NAME} and ${REPEATS} repeats"
...
if [ $SYSTEM_ONLY != "1" ]; then
  ...
  do
    echo "Start server for repeat model metrics ${repeat} num clients ${NUM_CLIENTS} and noise ${NOISE}  num rounds ${NUM_ROUNDS} and data ${DATA_NAME}"
    python server.py --data_path $DATA_PATH --run_repeat $repeat --num_clients $NUM_CLIENTS --num_rounds $NUM_ROUNDS --noise $NOISE &
    sleep  $(($NUM_CLIENTS * 2))
    ...
    do
      echo "Start client ${i}"
      client_index=$(($i -1))
      python client.py --client_index $client_index --data_path $DATA_PATH --run_repeat $repeat --noise $NOISE &
    done
    wait
    echo "Repeat model metrics ${repeat} num clients ${NUM_CLIENTS} and noise ${NOISE}  num rounds ${NUM_ROUNDS} and data ${DATA_NAME} complete"
    echo "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<"
  done
  echo "---------------------------------------------------------------------------------------------------------"
fi
if [ $SYSTEM_ONLY != "2" ]; then
  ...
  do
    ...
    python server.py --data_path $DATA_PATH --run_repeat $repeat --num_clients $NUM_CLIENTS --num_rounds $NUM_ROUNDS --system_metrics true --noise $NOISE &
    server_id=$!
    sleep 25
    echo "Start client"
    python client.py --client_index 1 --data_path $DATA_PATH --run_repeat $repeat --system_metrics true --noise $NOISE &
    client_id=$!
    for ((i=1;i<=$NUM_CLIENTS;i++))
    do
      echo "Start client ${i}"
    ...
    python ../scripts/mem_data_to_wandb.py --logs_path $client_time_logs --project_name $project_name --run_name $run_name --group_name $group_name  --memory_type "client"
    python ../scripts/mem_data_to_wandb.py --logs_path $server_time_logs --project_name $project_name  --run_name $run_name --group_name $group_name  --memory_type "server"
    echo "Repeat system metrics ${repeat} ${group_name} num rounds ${NUM_ROUNDS} and data ${DATA_NAME} complete"
    echo "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<"
  done
fi
