...
logging.basicConfig(
    filename="/home/lseeger/PycharmProjects/BenchmarkingFederated/example.log",
    encoding="utf-8",
    level=logging.DEBUG,
)
...
def build_model_delta_update_with_tff_optimizer(
    ...
    *,
    ...
    def client_update(optimizer, initial_weights, data, optimizer_hparams=None):
        ...
        # tff.federated_mean?
        client_update, has_non_finite_delta = tensor_utils.zero_all_if_any_non_finite(
            client_update
        )
        ...
        tf.print("Time for one client update is")
        tf.print(end - begin)
...
def build_model_delta_update_with_keras_optimizer(
    ...
    def client_update(optimizer, initial_weights, data):
        ...
        def reduce_fn(num_examples_sum, batch):
            ...
            # train_time = num_example_train_times[1]
            # num_examples_sum = num_example_train_times[0]
        ...
        tf.print(
            "Client training time", output_stream=f"file://{tff_time_logging_directory}"
        )
...
def build_model_delta_client_work(
    ...
    *,
    ...
    if not (isinstance(optimizer, optimizer_base.Optimizer) or callable(optimizer)):
        raise TypeError(
    ...
    return client_works.ClientWorkProcess(
        init_fn, next_fn, get_hparams_fn=get_hparams_fn, set_hparams_fn=set_hparams_fn
    )


def build_functional_model_delta_update(
    model: functional.FunctionalModel, *, weighting: client_weight_lib.ClientWeighting
) -> tf.function:
    ...
    def client_update_fn(optimizer, initial_weights, dataset):
        ...
        trainable_tensor_specs = tf.nest.map_structure(
            tf.TensorSpec.from_tensor, tuple(tf.nest.flatten(initial_trainable_weights))
        )
        optimizer_state = optimizer.initialize(trainable_tensor_specs)
