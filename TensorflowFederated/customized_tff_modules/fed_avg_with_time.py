...
import TensorflowFederated.customized_tff_modules.composers_with_time as composers
from tensorflow_federated.python.learning.templates import distributors
from tensorflow_federated.python.learning.templates import learning_process
import TensorflowFederated.customized_tff_modules.model_delta_client_work_time as model_delta_client_work_time
...
def build_weighted_fed_avg(
    ...
    *   `set_model_weights`: A `tff.Computation` with type signature
    ...
    py_typecheck.check_type(
        client_weighting, client_weight_lib.ClientWeighting, label="client_weighting"
    )
    ...
    if not callable(model_fn):
        client_work = (
            model_delta_client_work_time.build_functional_model_delta_client_work(
                model=model_fn,
        ...
        client_work = model_delta_client_work_time.build_model_delta_client_work(
            model_fn=model_fn,
            optimizer=client_optimizer_fn,
            client_weighting=client_weighting,
            metrics_aggregator=metrics_aggregator,
    ...
    return composers.compose_learning_process(
        initial_model_weights_fn, model_distributor, client_work, aggregator, finalizer
    )
...
def build_unweighted_fed_avg(
    ...
    *   `set_model_weights`: A `tff.Computation` with type signature
    ...
    py_typecheck.check_type(model_aggregator, factory.UnweightedAggregationFactory)
