...
from tensorflow_federated.python.learning.models import (
    model_weights as model_weights_lib,
)
...
import TensorflowFederated.customized_tff_modules.learning_process_with_time as learning_process
...
def compose_learning_process(
    ...
    def next_fn(state, client_data):
        ...
        times["finalize_time_end"] = timestamp()

        new_global_model_weights = finalizer_output.result
        times["create_new_state_begin"] = timestamp()
        ...
        times["create_new_state_end"] = timestamp()
        times["create_metrics_begin"] = timestamp()
        ...
        times["create_metrics_end"] = timestamp()
        return learning_process.LearningProcessOutput(new_state, metrics, times)
...
def _validate_args(
    ...
    py_typecheck.check_type(initial_model_weights_fn, computation_base.Computation)
    ...
    global_model_weights_type = computation_types.at_server(global_model_weights_type)
    py_typecheck.check_type(model_weights_distributor, distributors.DistributionProcess)
...
def build_basic_fedavg_process(
    ...
    return compose_learning_process(
