...
def _attr_bool_check(instance, attribute, value):
    if not isinstance(value, tf.Tensor):
        if not isinstance(value, (bool, bool, np.bool_)):
            raise ValueError(
                f"`{attribute.name}` should be a bool constant. Found "
...
class CompressionSumQuery(tfp.SumAggregationDPQuery):
    ...
    def __init__(
        ...
        inner_query: tfp.SumAggregationDPQuery,
        ...
        if not isinstance(inner_query, tfp.SumAggregationDPQuery):
            raise ValueError(
                "`inner_query` should be a `SumAggregationDPQuery`. "
    ...
    def set_ledger(self, ledger):
        raise NotImplementedError("Ledger has not yet been implemented for this query!")
    ...
    def initial_global_state(self):
        return self._GlobalState(
            new_seed_pair(),
            self._quantization_params.to_tf_tensors(),
    ...
    def derive_sample_params(self, global_state):
        return self._SampleParams(
            global_state.sample_hadamard_seed,
            global_state.quantization_params,
    ...
    def preprocess_record(self, params, record):
        ...
        encoded_record = self._encode_record(
            record, params.sample_hadamard_seed, params.quantization_params
        )
        return self._inner_query.preprocess_record(
            params.inner_query_params, encoded_record
        )
