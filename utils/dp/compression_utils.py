...
def stochastic_rounding(x, conditional, l2_norm_bound=None, beta=DEFAULT_BETA):
    ...
    def post_rounding_l2_norm_bound(x, l2_norm_bound, beta):
        ...
        squared_bound2 += tf.sqrt(2.0 * tf.math.log(1.0 / beta)) * (
            x_norm + 0.5 * tf.sqrt(dim)
        )
    ...
    def round_fn(repeat, _):
        ...
        repeat = tf.logical_and(
            conditional, tf.greater(rounded_l2_norm, l2_norm_threshold)
        )
...
def scaled_quantization(
    x, scale, stochastic, conditional, l2_norm_bound, beta=DEFAULT_BETA
):
...
def randomized_hadamard_transform(x, seed_pair, repeat=1):
    ...
    def apply_transform(repeat_index, x):
        ...
        encoded_x = tf.squeeze(
            fast_walsh_hadamard_transform(tf.expand_dims(rademacher_x, axis=0)), axis=0
        )
        return encoded_x
...
def fast_walsh_hadamard_transform(x):
    ...
    with tf.compat.v1.name_scope(None, "fast_walsh_hadamard_transform"):
        ...
        h_core = tf.constant(
            [[1.0, 1.0], [1.0, -1.0]], dtype=x.dtype, name="hadamard_weights_2x2"
        )
