...
#         "_GlobalState", ["l2_norm_bound", "local_stddev"]
#     )
#
#     # pylint: disable=invalid-name
#     _SampleParams = collections.namedtuple(
#         "_SampleParams", ["l2_norm_bound", "local_stddev"]
#     )
#
#     def __init__(
#         self,
...
#         return self._SampleParams(global_state.l2_norm_bound, global_state.local_stddev)
#
...
#
#         self._discretize_struct(
#             record_as_list,
#             self.scale_factor,
#             self.stochastic,
#             self.beta,
#             self.prior_norm_bound,
#         )
...
#             lambda x: tf.compat.v1.assert_type(x, tf.int32), record_as_list
#         )
#         dependencies = [
...
class LocalGaussianSumQuery(dp_query.SumAggregationDPQuery):
    """Implements DPQuery for adding local noise to each trient after training.

    For each local record, we check the L2 norm bound and add Gaussian
    noise. In particular, this DPQuery does not perform L2 norm clipping and the
    norms of the input records are expected to be bounded.
    """
    ...
    def derive_sample_params(self, global_state):
        """
        Derives sample params from global state
        """
        return self._SampleParams(global_state.l2_norm_bound, global_state.local_stddev)
    ...
    def preprocess_record(self, params, record):
        ...
        record, _ = tf.clip_by_global_norm(record, params.l2_norm_bound)
        result = tf.cond(
            tf.equal(params.local_stddev, 0),
            lambda: record,
            lambda: self._add_local_noise(record, params.local_stddev),
    ...
    def get_noised_result(self, sample_state, global_state):
        """
        Gets the noised result from state
        """
        event = dp_accounting.UnsupportedDpEvent()
        return sample_state, global_state, event
