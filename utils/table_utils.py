...
def compute_summary_columns(df):
    """
    ...
    for index, row in df.iterrows():
        ...
        df.at[
            index, "group summary tff"] = f"{np.mean(tff_means):.4f} \u00B1 {np.sqrt(np.sum(np.array(tff_stds) ** 2)):.4f}"
        df.at[
            index, "group summary flwr"] = f"{np.mean(flwr_means):.4f} \u00B1 {np.sqrt(np.sum(np.array(flwr_stds) ** 2)):.4f}"
    return df
