...
def log_df_info(df, column):
    ...
    logging.info(
        f"Counts of unique values in column '{column}':\n{unique_value_counts}"
    )
...
def select_feature_genes(df: pd.DataFrame, cov_threshold=3.5):
    ...
    median_df = pd.DataFrame(
        np.zeros((len(cell_types), df.shape[1]))
    )  # 75 rows × 13945 columns
...
def create_brain_cell_patient_partitions():
    ...
    sample_columns = pd.read_csv(
        "../Dataset2/human_MTG_gene_expression_matrices_2018-06-14/human_MTG_2018-06-14_samples-columns.csv"
    )
    sample_columns = sample_columns[sample_columns["label"] != "no class"]
    client_one_row_numbs = list(
        sample_columns[sample_columns["donor"] == "H200.1030"].index
    )
    client_one_row_numbs.append(0)
    client_two_row_numbs = list(
        sample_columns[sample_columns["donor"] == "HH200.1023"].index
    )
    client_two_row_numbs.append(0)
    client_three_row_numbs = list(
        sample_columns[
            sample_columns["donor"] != "HH200.1023"
            and sample_columns["donor"] != "H200.1030"
        ].index
    )
...
def create_X_y_from_gen_df(
    df: pd.DataFrame, scaling=True, label=configs["label"]
...
def preprocess(
    dataset: tf.data.Dataset, epochs: int = configs.get("epochs"), seed: int = 1
):
    set_random_seed(seed)
    return (
        dataset.shuffle(configs.get("shuffle"), seed=1, reshuffle_each_iteration=True)
        .batch(configs.get("batch_size"))
        .repeat(epochs)
    )


def create_class_balanced_partitions(
    df: pd.DataFrame, num_partitions: int, label=configs["label"]
):
    """
    ...
    """
    partitioner = StratifiedKFold(
        n_splits=num_partitions,
        shuffle=True,
        random_state=configs["random_state_partitions"],
    )
    partition_rows = []
...
def create_unbalanced_splits(df: pd.DataFrame, label_name: str, unweight_step: int):
    ...
    partition_size = floor(
        min([class_size * len(df) for class_size in class_percentages]) - minus_value
    )
    ...
    other_classes_percentage = round(
        (1 - chosen_class_percentage) / (num_classes - 1), 8
    )
    ...
    partitions_list = []
    downsampled_df = df.groupby(label_name, group_keys=False).apply(
        lambda x: x.sample(partition_size, random_state=69)
    )
    ...
    for chosen_class in classes:
        chosen_class_indices = (
            downsampled_df[downsampled_df[label_name] == chosen_class]
            .sample(chosen_class_samples, random_state=69)
            .index.tolist()
        )
        if other_classes_percentage == 0.0:
            ...
            other_classes_indices = (
                ...
                .apply(
                    lambda x: x.sample(
                        other_classes_samples, random_state=69
                    ).index.tolist()
                )
                .explode()
                .tolist()
        ...
        for class_name in classes:
            if class_name == chosen_class:
                ...
                partitions_dict["class " + str(class_name)].append(
                    other_classes_samples
                )

    for count, i in enumerate(range(69, 69 + 10)):
        test_df = remaining_df.groupby(label_name, group_keys=False).apply(
            lambda x: x.sample(floor(test_sample_size / num_classes), random_state=i)
        )
        test_df.to_csv(
            configs["data_directory"] + "unweighted_test_df_" + str(count) + ".csv",
            index=False,
        )
