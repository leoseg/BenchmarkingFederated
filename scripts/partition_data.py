...
from utils.data_utils import (
    create_class_balanced_partitions,
    create_unbalanced_splits,
    load_data,
    preprocess_data,
)
import argparse
...
def main():
    ...
    if args.unweighted_step < 0:
        partitions = create_class_balanced_partitions(
            df, num_partitions=args.num_clients
        )
        with open("partitions_list", "wb") as file:
            pickle.dump(partitions, file)
    else:
        class_num_df, partitions = create_unbalanced_splits(
            df, label_name=args.label_name, unweight_step=args.unweighted_step
        )
        # Create charts and dictionary of number of samples per class and client and saves them
        class_num_df.plot(
            kind="bar", stacked=True, xlabel="Clients", ylabel="Num examples"
        )
        ...
        plt.savefig(
            f"../plots/class_imbalance_{percentage}_clients_{args.num_clients}.png"
        )
        with open("partitions_list", "wb") as file:
            pickle.dump(partitions, file)
