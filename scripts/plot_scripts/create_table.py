from datapostprocessing.table_utils import (
    create_summarize_dataframe_from_metrics,
    transform_central_metric_to_df,
    transform_df_to_landscape_table,
    get_usecase_name,
...
for usecase in [1, 2, 3, 4]:
    ...
    for mode in ["system", "balanced", "unweighted"]:
        ...
        data = mongodb.get_data_by_name(
            f"scenario_metrics_{usecase}_{mode}", calc_total_memory=True
        )
        appendixstr += "\\begin{landscape}\n "
        for metric, metric_name in get_metrics_for_mode(mode, usecase):
            if metric in ["sent", "received"]:
                data2 = mongodb.get_data_by_name(
                    f"scenario_metrics_{usecase}_system_network", calc_total_memory=True
                )
                df = create_summarize_dataframe_from_metrics(
                    data=data2, metric_name=metric, rounds=[1, 3, 5, 10], groups=groups
                )
                central_df = None
            else:
                df = create_summarize_dataframe_from_metrics(
                    data=data, metric_name=metric, rounds=[1, 3, 5, 10], groups=groups
                )
            if mode == "unweighted" or metric in ["sent", "received"]:
                ...
                central_df = transform_central_metric_to_df(
                    central, metric_name=central_metric
                )
            if "memory" in metric:
                cols = [col for col in df.columns if col != "group"]
                for col in cols:
                    df[col] = df[col].apply(convert_val)
            appendixstr += transform_df_to_landscape_table(
                df,
                headers,
                f"{metric_name} for {get_usecase_name(usecase)}",
