...
for mode in ["system"]:
    ...
    match mode:
        ...
        case "system":
            ...
            metrics_tuples.append(("total_per_client_memory_client","Memory per client in MB"))
            metrics_tuples.append(("total_client_time","Client training time in seconds"))



    for metric,metric_name in metrics_tuples:
        if metric == "sent":
            scenario_metrics = mongodb.get_data_by_name(f"scenario_metrics_{configs.get('usecase')}_{mode}_network",True)
            ...
            plot_swarmplots(df1,metric_name=metric_name,data_path=plot_path,plot_type="bar",scale=None,data2=df2,configuration_name="Number of clients")
            continue
        ...
        for plot_type in ["bar"] :
            if metric_name == "AUC":
                ...
                scale = None
            plot_swarmplots(df,metric_name,configuration_name,data_path=plot_path,plot_type=plot_type,scale=scale)
