...
from dp.dp_models import get_model
from sklearn.model_selection import StratifiedKFold, train_test_split
import wandb
from wandb.keras import WandbCallback
from utils.config import configs
import argparse
from dp.dp_utils import GlobalNorm
...
for noise in [20.0]:
    ...
    model.compile(
        optimizer=optimizer, loss=configs.get("dp_loss"), metrics=configs.get("metrics")
    )
    ...
    history = model.fit(
        ...
        validation_freq=configs["valid_freq"],
        validation_split=0.1,
        callbacks=[wandb_callback, global_norm],
    )
    ...
    for count, (train, test) in enumerate(kfold.split(X, Y)):
        global_norm = GlobalNorm()
        print(global_norm.median_global_norm)
        wandb.init(
            project=project_name,
            config=configs,
            group=group_name,
        ...
        model.compile(
            optimizer, loss=configs.get("dp_loss"), metrics=configs.get("metrics")
        )
        model.fit(
            X_train,
            Y[train],
