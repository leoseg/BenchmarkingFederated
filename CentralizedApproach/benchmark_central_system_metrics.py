...
parser.add_argument("--run_repeat", type=int, help="num of repeat", default=1)
parser.add_argument(
...
kfold = StratifiedKFold(
    n_splits=configs.get("n_splits"), shuffle=True, random_state=args.run_repeat
)
...
for count, (train, test) in enumerate(kfold.split(X, Y)):
    ...
    wandb.init(
        ...
        config=configs,
        job_type="train",
        group=f"usecase_{configs['usecase']}",
        name=f"repeat_{args.run_repeat}",
    )
    ...
    model = get_model(
        input_dim=X.iloc[train].shape[1],
        num_nodes=num_nodes,
