...
parser.add_argument(
    "--num_nodes",
    type=int,
    help="num of nodes for each layer",
    default=configs.get("num_nodes"),
)
...
parser.add_argument(
    ...
    type=float,
    help="dropout rate",
    default=configs.get("dropout_rate"),
)
parser.add_argument(
...
data_pathes = [
    "../DataGenExpression/Dataset1.csv",
    "../DataGenExpression/Dataset2.csv",
    "../DataGenExpression/Dataset3.csv",
]
Xs = []
Ys = []
...
kfold = StratifiedKFold(
    n_splits=configs.get("n_splits"), shuffle=True, random_state=random_state
)
...
for count, ((train, test), (train2, test2), (train3, test3)) in enumerate(
    zip(kfold.split(Xs[0], Ys[0]), kfold.split(Xs[1], Ys[1]), kfold.split(Xs[2], Ys[2]))
):
    X_train = pandas.concat(
        [Xs[0].iloc[train], Xs[1].iloc[train2], Xs[2].iloc[train3]], ignore_index=True
    )
    Y_train = pandas.concat(
        [Ys[0][train], Ys[1][train2], Ys[2][train3]], ignore_index=True
    )
    X_test = pandas.concat(
        [Xs[0].iloc[test], Xs[1].iloc[test2], Xs[2].iloc[test3]], ignore_index=True
    )
    Y_test = pandas.concat([Ys[0][test], Ys[1][test2], Ys[2][test3]], ignore_index=True)

    wandb.init(
        project=f"choose-best-config-central_Alldata_gen_expr",
        config=configs,
        group=f"even_split_crossfold_random_state_{random_state}_{num_nodes}_dropout_{dropout_rate}_l1_{l1_v}",
        job_type="train",
        name=f"k_fold_{count}",
    )
    ...
    model = get_seq_nn_model(
        X_train.iloc[train].shape[1], num_nodes, dropout_rate, l1_v, configs.get("l2_v")
    )
    model.compile(
